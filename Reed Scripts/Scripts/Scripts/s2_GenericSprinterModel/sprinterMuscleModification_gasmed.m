function sprinterMuscleModification_gasmed(model,notes)

% move GasMed_at_shank inferiorly until no longer penetrated by mtu at max
% knee flexion
...
% get gasmed
gasmed = model.getMuscles.get('gasmed_r');

% get geometry path and origin/insertion
path = GeometryPath.safeDownCast(gasmed.getGeometryPath);
...
% get GasMed_at_shank_r
for k = 0:path.getWrapSet.getSize-1
    if strcmp(path.getWrapSet.get(k).get_wrap_object,'GasMed_at_shank_r')
        cyl = path.getWrapSet.get(k).getWrapObject;
        break;
    end
    if k == path.getWrapSet.getSize-1; error('could not find GasMed_at_shank_r'); end
end
...
fprintf('-cylinder GasMed_at_shank_r moved inferiorly %4.2f cm\n',total_decrement*100);
if nargin > 1
    fprintf(notes,'-cylinder GasMed_at_shank_r moved inferiorly %4.2f cm\n',total_decrement*100);
end
...
% get gaslat
gasmed = model.getMuscles.get('gasmed_l');
...
% get GasMed_at_shank_l
for k = 0:path.getWrapSet.getSize-1
    if strcmp(path.getWrapSet.get(k).get_wrap_object,'GasMed_at_shank_l')
        cyl = path.getWrapSet.get(k).getWrapObject;
        break;
    end
    if k == path.getWrapSet.getSize-1; error('could not find GasMed_at_shank_l'); end
end
...
if abs(lenpath - lenp2p) < eps
    error('cylinder penetrated for left gasmed using cylinder modifications that successfully disallowed penetration for right gasmed');
end
