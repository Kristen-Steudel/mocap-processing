function sprinterMuscleModification_vasint(model,notes)
...
% get muscle
vi = model.getMuscles.get('vasint_r');

% get path points and their frames
path = GeometryPath.safeDownCast(vi.getGeometryPath);
...
pnew.setName(['vasint_r-P' num2str(segtouch) 'a'])
...
if nargin > 1
    fprintf(notes,'-vasint modification:\n');
...
% get muscle
vi = model.getMuscles.get('vasint_l');
...
% create new conditional path point
pnew = ConditionalPathPoint;
pnew.setName(['vasint_l-P' num2str(segtouch) 'a'])
pnew.setRangeMin(qstart)
pnew.setRangeMax(pi);
pnew.setCoordinate(coord);
