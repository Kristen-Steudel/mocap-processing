function sprinterMuscleModification_vasmed(model,notes)
...
% get muscle
vm = model.getMuscles.get('vasmed_r');

% get path points and their frames
path = GeometryPath.safeDownCast(vm.getGeometryPath);
...
pnew.setName(['vasmed_r-P' num2str(segtouch) 'a'])
...
if nargin > 1
    fprintf(notes,'-vasmed modification:\n');
...
% get muscle
vm = model.getMuscles.get('vasmed_l');
...
% create new conditional path point
pnew = ConditionalPathPoint;
pnew.setName(['vasmed_l-P' num2str(segtouch) 'a'])
pnew.setRangeMin(qstart)
pnew.setRangeMax(pi);
pnew.setCoordinate(coord);
