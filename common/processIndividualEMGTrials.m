function [] = processIndividualEMGTrials(FILT,muscleNames,pathNames)
...
%    end
   ...
   NORM_EMG = EMGlp./NORM.maxAct;

   NORM_EMG_Struct = [];
   NORM_EMG_Struct.NORM_EMG = NORM_EMG;
   NORM_EMG_Struct.muscleNames = muscleNames;

   %cd('C:15086\Documents\GitHub\mocap-processing\S001\S001_anc_files')
   newFileName = replace(pathNames{fnum},'.anc','_EMG.mat');  % New name for the saved structure
   save(newFileName, 'NORM_EMG_Struct');  % Save the structure with the new name
...
%cd('C:15086\Documents\GitHub\mocap-processing\S001\S001_anc_files')
%newFileName = 'S001Act.mat';  % New name for the saved structure
